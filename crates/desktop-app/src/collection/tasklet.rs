...
pub async fn on_settings_changed(
    ...
    let mut settings_state_sub = if_none_return!(settings_state.upgrade()).subscribe();
    log::debug!("Starting on_settings_changed_update_state");
    loop {
        {
            let settings_state = if_none_break!(settings_state.upgrade());
            let observable_state = if_none_break!(observable_state.upgrade());
            let handle = if_none_break!(handle.upgrade());
