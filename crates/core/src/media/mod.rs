...
pub struct Source {
    ...
    /// Last synchronization with external source
    ///
    /// The time stamp of the last known synchronization with an external
    /// provider or `None` if unsynchronized.
    ///
    /// WARNING: This time stamp should only be compared with other time
    /// stamps from the same external source provider! Consistency is only
    /// guaranteed among time stamps from the same clock source.
    /// Example: Modification time stamps generated by file systems might
    /// not be (exactly) synchronized with the local system clock.
