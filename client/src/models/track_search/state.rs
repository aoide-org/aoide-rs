...
impl State {
    ...
    pub fn search_params_for_fetching_results(&self) -> Option<&SearchParams> {
    ...
    pub(super) fn append_fetched_result_page(&mut self, response: FetchResultPageResponse) {
        ...
        debug_assert_eq!(self.results.len(), pagination.offset);
        debug_assert!(entities.len() <= pagination.limit.get());
        self.control_state = if entities.len() < pagination.limit.get() {
            // Final page
            ControlState::Done
        } else {
