...
use crate::{modify_shared_state_infallible, Reaction, StateEffect};
...
impl State {
    ...
    fn update_music_dir(&mut self, music_dir: Option<&DirPath<'_>>) -> StateEffect {
...
impl SharedState {
    ...
    pub fn update_music_dir(&self, music_dir: Option<&DirPath<'_>>) -> StateEffect {
        modify_shared_state_infallible(&self.0, |state| {
            (Reaction::Accepted, state.update_music_dir(music_dir))
        })
        .1
    }
}
